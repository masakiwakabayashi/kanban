    ...
    <div class="container">
        <form class="mt-5, signupForm" id="new_user" action="{{ route('register') }}" accept-charset="UTF-8" method="post">
            ...
            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                ...
                <input id="user_name" class="form-control" placeholder="名前を入力してください" type="text" name="name" value="{{ old('name') }}" required autofocus>
                @if ($errors->has('name'))
            ...
            <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                ...
                <input id="user_email" class="form-control" placeholder="emailを入力してください" autocomplete="email" type="email" name="email" value="{{ old('email') }}" required>
                @if ($errors->has('email'))
                    <span class="help-block">
